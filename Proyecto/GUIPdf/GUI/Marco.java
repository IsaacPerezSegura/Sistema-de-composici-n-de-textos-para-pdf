...
if(event.getSource()==items[0]){
    ...
    if(posicion>0 || v==true){
     ...
     panel[posicion].abrirTxt();
     frameInterno.setName(String.valueOf(posicion));
     anadirFrameInterno(panel[posicion]); 
     posicion=0;
     v=false;
    ...
    if(event.getSource()==items[1]){
        ...
        if(posicion>0 || v==true){
         ...
         panel[cont]= new Panel();
         titulo=panel[cont].abrir();
         if(!titulo.isEmpty() /*&& titulo.endsWith(".pdf")*/){
         frameInterno=new JInternalFrame(titulo,true,true,true,true);
         frameInterno.setName(String.valueOf(cont));
         anadirFrameInterno(panel[cont]);
         }
...
if(guardar>-1){
    ...
    JOptionPane.showMessageDialog(null, "Isaac P\u00e9rez Segura\n" +
...
public void anadirFrameInterno(Panel p){
...
private class internalFrameListener implements InternalFrameListener {
        ...
        public void internalFrameActivated(InternalFrameEvent e) {
            ...
            if(!e.getInternalFrame().getTitle().endsWith(".pdf")){
